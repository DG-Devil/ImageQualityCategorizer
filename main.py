...
def is_blurry(path, threshold=60):
...
# --- Updated Show Results with Quality Badge ---
def show_results():
    for w in content_frame.winfo_children():
        w.destroy()
    ...
    for cat, files in sorted_categories:
        frame = Frame(content_frame, bg="#f9f9f9", bd=2, relief="groove")
        frame.pack(fill="x", pady=15, padx=10)

        # --- Quality Badge (Top Right) ---
        if "4K" in cat:
            q = 100 if "Clear" in cat else 99
        elif "1440" in cat:
            q = 99 if "Clear" in cat else 95
        elif "1080" in cat:
            q = 99 if "Clear" in cat else 95
        elif "720" in cat:
            q = 95 if "Clear" in cat else 85
        elif "480" in cat:
            q = 80 if "Clear" in cat else 70
        elif "360" in cat:
            q = 65 if "Clear" in cat else 55
        else:
            q = 60 if "Clear" in cat else 50

        badge_color = "#4CAF50" if "Clear" in cat else "#FF9800"

        Label(frame, text=cat, font=("Arial", 11, "bold"), bg="#f9f9f9").place(x=10, y=8)
        badge = Label(frame, text=f"Quality: {q}%", bg=badge_color, fg="white",
                      font=("Arial", 10, "bold"), padx=10, pady=2)
        badge.place(relx=1.0, x=-10, y=8, anchor="ne")

        # Download Button
        btn = Button(
            frame, text=f"Download ZIP",
            bg="#8C00FF", fg="white",
            font=("Arial", 10, "bold"),
            padx=15, pady=6,
        ...
        imgs_frame = Frame(frame, bg="#f9f9f9")
        ...
        for name, path in files:
            try:
                ...
                lbl = Label(imgs_frame, image=tk_img, bg="#f9f9f9")
                lbl.image = tk_img
                lbl.grid(row=row * 2, column=col, padx=5, pady=5)

                lbl_name = Label(imgs_frame, text=name, font=("Arial", 9), bg="#f9f9f9")
...
title = Label(root, text="Image Quality Categorizer", font=("Arial", 16, "bold"),
              bg="#8C00FF", fg="white", pady=12)
title.pack(fill=X)

btn_frame = Frame(root, bg="#f2f2f2")
btn_frame.pack(pady=10)

Button(btn_frame, text="📸 Select Images", bg="#8C00FF", fg="white",
       relief=GROOVE, command=analyze_selected_images).pack(side=LEFT, padx=10)
Button(btn_frame, text="📁 Select Folder", bg="#8C00FF", fg="white",
       relief=GROOVE, command=analyze_folder).pack(side=LEFT, padx=10)
